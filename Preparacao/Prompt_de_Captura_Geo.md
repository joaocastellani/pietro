# PROMPT DE CAPTURA — GEOGRAFIA (9º ano)
# Arquivo gerado: geo-[unidade]-[capitulo].md

---

Tarefa: Extraia todo o conteúdo desta página de Geografia e gere um
...
- Para mapas: capture todos os dados legíveis — países, capitais,
  coordenadas, escalas, legendas, valores numéricos associados
- IMAGENS ANEXADAS: se imagens da apostila forem anexadas junto
  com este prompt, extraia o conteúdo textual visível nelas —
  legendas de mapas, dados de cartogramas, títulos de gráficos,
  rótulos de regiões, valores de escalas — e incorpore nas seções
  correspondentes. Para mapas: capture toda informação da legenda
  e dos rótulos visíveis. Não ignore imagens.
...
cujo conteúdo não foi possível extrair por texto (ex: mapa sem
legenda legível, tabela em formato de imagem, cartograma com
gradiente), adicione ao final do arquivo:
...
Ex:
## IMAGENS RECOMENDADAS PARA CAPTURA
- Mapa político da Europa do Sul · países e capitais da região ·
  necessário para completar tabela de regiões europeias
- Tabela de IDH por país europeu · dados de IDH e ranking ·
  necessário para warm-up e questões de comparação
...
- Alterne sentidos: conceito→definição, país→capital,
  região→característica, dado→país/região
...
**FC-2**
🔵 Frente: País mais populoso da América do Sul.
🟢 Verso: Brasil, com mais de 200 milhões de habitantes.

---

## BLOCO A — MAPAS E REPRESENTAÇÕES CARTOGRÁFICAS
[Execute se o capítulo apresentar mapas, cartogramas, plantas
ou qualquer representação espacial com legenda ou dados.
IMPORTANTE: se imagens de mapas forem anexadas, cada mapa
deve gerar uma entrada neste bloco — não ignore mapas anexados.]

Para cada mapa ou representação:
- Título do mapa (conforme o material)
- Tipo: [político / físico / temático / cartograma /
  climático / econômico / demográfico / outro]
- Legenda: capture TODOS os elementos da legenda com seus
  significados e valores exatos
  Ex: "> 1 milhão (vermelho escuro) · 500.001–1 milhão
  (vermelho) · 100.001–500.000 (laranja escuro)..."
- Países/regiões visíveis: liste todos os identificáveis
- Escala (se presente): valor exato conforme o material
- Fonte (se citada no material)
- Contexto de uso: o que o mapa ilustra no capítulo

---

## BLOCO B — DADOS GEOGRÁFICOS ESTRUTURADOS
[Execute se o capítulo apresentar tabelas, gráficos ou listas
com dados geográficos mensuráveis]
...
- Gráficos ou cartogramas — capture apenas os valores
  explicitamente marcados nos eixos ou rótulos; nunca reconstitua
  uma tabela a partir da forma visual do gráfico
- Legendas de mapas ou imagens
...
- Capture apenas o que está literalmente impresso no gráfico
